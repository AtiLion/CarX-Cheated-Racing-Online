...
namespace CheatInstaller
{
    ...
    {
        #region Info
        public static readonly string CheatName = "CarXCheatedRacingOnline.dll";
        public static readonly string ExePath = "/Drift Racing Online.exe";
        public static readonly string ManagedPath = "/Drift Racing Online_Data/Managed";
        public static readonly string AssemblyCSharpPath = ManagedPath + "/Assembly-CSharp.dll";
        ...
        {
            Console.Title = "CarXCheatedRacingOnline by AtiLion";

            Console.WriteLine("CarXCheatedRacingOnline - A cheat/hack for the game CarX Drift Racing Online.");
        ...
        private void Run()
        {
            Console.WriteLine("Enter location to the CarX Drift Racing Online directory!");
            do
            {
        ...
        private void Install()
        {
            ...
            try
            {
                ...
                TypeDef typLoader = asmHack.GetTypes().FirstOrDefault(a => a.FullName == "CarXCheatedRacingOnline.Loader");
                if(typLoader == null)
                {
                    Console.WriteLine("Cheat loader class not found!");
                    Exit();
                }
                TypeDef typMainMenu = asmAssembly.GetTypes().FirstOrDefault(a => a.FullName == "GameManager");
                if (typMainMenu == null)
                {
                    Console.WriteLine("Game GameManager class not found!");
                    Exit();
                }
                MethodDef mthHook = typLoader.Methods.FirstOrDefault(a => a.Name == "CallMeToHook" && a.IsPublic && a.IsStatic);
                if(mthHook == null)
                {
                    Console.WriteLine("Cheat hook method not found!");
                    Exit();
                }
                MethodDef mthStart = typMainMenu.Methods.FirstOrDefault(a => a.Name == "Awake" && a.IsPrivate && !a.IsStatic);
                if(mthStart == null)
                {
                    Console.WriteLine("Game Awake method not found!");
                    Exit();
                }
                ...
                ITypeDefOrRef typrefLoader = importer.Import(new TypeRefUser(asmHack, "CarXCheatedRacingOnline", "Loader"));
